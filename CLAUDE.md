...
- **Fase 8** (`phase2/missing-details`, branch actual) 🚧 — admin cross-user reads + schedule por día + email template bilingüe.
  ...
  - **Schedule per-day** (commit `5e5dd1b`): tabla `artisan_schedules` rediseñada a `artisan_schedule_days(artisan_id, weekday)` PK compuesta. Cada artista tiene 7 entradas (lun-dom), cada una con su propio `is_working` + `start_time` + `end_time`. Migration `0008_schedule_per_day.sql` migra datos existentes. UI en `SchedulesSection` muestra una fila por día con time pickers independientes. `buildSchedule()` ahora consume el schedule real para calcular dayOff y rango de slots (antes era seed determinista mock).
  - **Email template bilingüe** (`supabase/email-templates/magic-link.html`): HTML con estilos inline DSR (Noir + dorado, serif italic, monospace para el código) y bloque ES + bloque EN apilados. Se pega manualmente en Supabase Dashboard → Auth → Email Templates → Magic Link. Variables soportadas: `{{ .Token }}`, `{{ .ConfirmationURL }}`, `{{ .Email }}`.

> Próxima acción sugerida: aplicar migrations 0007 + 0008 en Supabase, pegar el template del magic link en el dashboard, y configurar Vercel.
